...
interface OverlaySegmentShape {
	...
	opacity: number;
	fadeDuration: number;
	animIn?: OverlayAnimValue | null;
	animOut?: OverlayAnimValue | null;
	animDuration: number;
}
...
export function OverlaysTab() {
	...
	const segments = (): OverlaySegmentShape[] => {
		const timeline = project.timeline as
			| ({ overlaySegments?: OverlaySegmentShape[] } & typeof project.timeline)
			| undefined;
		return timeline?.overlaySegments ?? [];
	};

	const writeSegments = async (next: OverlaySegmentShape[]) => {
	...
	const addOverlay = async (filePath: string) => {
		...
		const newSegment: OverlaySegmentShape = {
		...
		await writeSegments([...segments(), newSegment]);
	};
	...
	const updateSegment = async (
		id: string,
		patch: Partial<OverlaySegmentShape>,
	) => {
		await writeSegments(
			...
			<div class="flex flex-col gap-2">
				...
				>
					<ul class="flex flex-col gap-3">
						<For each={segments()}>
							{(seg) => (
								<li class="flex flex-col gap-2 rounded border border-gray-3 p-2">
...
function AnimField(props: {
	label: string;
	value: string;
	onChange: (value: OverlayAnimValue | null) => void;
}) {
	return (
		<label class="flex flex-col gap-0.5">
			<span class="text-[0.65rem] text-gray-11">{props.label}</span>
			<select
				...
				onChange={(e) => {
					...
					props.onChange(v === "" ? null : (v as OverlayAnimValue));
				}}
				class="w-full rounded border border-gray-3 bg-gray-1 px-2 py-1 text-xs text-gray-12"
			>
